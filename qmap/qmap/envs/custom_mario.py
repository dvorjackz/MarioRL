...
from qmap.qmap.utils.lazy_frames import LazyFrames
...
class CustomSuperMarioAllStarsEnv(Env):
    ...
    def __init__(self, screen_ratio=4, coords_ratio=4, use_color=True, use_rc_frame=True, stack=3, frame_skip=4, action_repeat=4, level='1.1'):
        ...
        self.frames = deque([], maxlen=(
            self.frame_skip * (self.stack - 1) + 1))
        self.observation_space = spaces.Box(low=0, high=255, shape=(
            self.screen_height, self.screen_width, n_frames))
        # coordinates
        ...
        # actions
        self.action_names = ['JUMP+LEFT', 'JUMP',
                             'JUMP+RIGHT', 'LEFT', 'NOOP', 'RIGHT']
    ...
    def _get_image(self):
        ...
        img = imresize(
            img, (self.screen_height, self.screen_width), interp='bilinear')
        if not self.use_color:
            img = img[:, :, None]
        return img
    ...
    def _get_obs(self):
        ...
        self.score = score
        if flag:
            # final bonus (seems to be 400 and then a series of 200 then 150 originaly)
            rew += 5000
        rew /= 100  # rescale
