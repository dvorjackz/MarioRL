...
class PerfLogger(Wrapper):
    def __init__(self, env, gamma, path):
        ...
        self.logger = CSVLogger(['steps', 'undiscounted return',
                                 'discounted return', 'episode length'], path + '/score')
    ...
    def reset(self):
        if self.ep_length != 0:
            self.logger.log(self.steps-self.ep_length, self.undiscounted_return,
                            self.discounted_return, self.ep_length)
